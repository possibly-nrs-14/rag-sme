...
def sanitize_for_injection(text, replacement_marker="[SANITIZED_INSTRUCTION]"):
    ...
    # 3. Neutralize detected patterns
    def replacer(match):
        # This function is called for every match.
        # We log the specific pattern that was hit for security auditing.
        # We find which group (pattern) was matched.
        matched_group_index = next(i for i, group in enumerate(match.groups()) if group is not None)
        matched_pattern = injection_patterns[matched_group_index]
        logging.warning(f"Sanitized potential injection pattern: '{match.group(0)}' (matched: '{matched_pattern}')")
        return replacement_marker

    sanitized_text = compiled_pattern.sub(replacer, text)
    
    # 4. Normalize spaces again after sanitization
